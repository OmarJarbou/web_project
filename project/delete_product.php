...
if (isset($data['delete_product_id'])) {
    $productId = intval($data['delete_product_id']);
    ...
    try {
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                if ($actualId != $currentId) {
                    ...
                    $stmtUpdate = $conn->prepare("UPDATE products SET id = ? WHERE id = ?");
        ...
        $conn->query("ALTER TABLE products AUTO_INCREMENT = $nextId");
        $conn->close();

        echo json_encode(['success' => true, 'message' => "Product ID $productId deleted successfully."]);
    } catch (Exception $e) {
